import { sha256, stableJson, } from '@ishuowang/rolehub-core';
import { HOST_TOOL_BINDINGS } from './types.js';
/** Capabilities for which this Host has a fixed, non-role-controlled implementation. */
export const HOST_SUPPORTED_CAPABILITIES = new Set(Object.keys(HOST_TOOL_BINDINGS));
/**
 ...
 */
export function createEffectivePolicy(role, allowedCapabilities) {
    ...
    const grants = role.manifest.spec.capabilities.required
        ...
        .filter(capability => (hostAllowed.has(capability)
    ...
    if (missing.length > 0) {
        throw new Error(`rolehub-bridge: Host policy cannot grant required capabilities: ${missing.join(', ')}`);
    }
    const receipt = {
...
export function bindingsForGrants(grants) {
    const bindings = {};
...
export function assertEffectivePolicy(role, policy, bindings) {
    ...
    if (!sameStrings(regenerated.policy.grants, policy.grants)
