...
import { CAPABILITY_IDS, stableJson, type CapabilityId } from '@ishuowang/rolehub-core'
...
export class RoleHubStorage {
  ...
  async readCatalogCache(hubId: string): Promise<unknown | undefined> {
    return readOptionalPrivateJson(
      path.join(this.catalogsDir, `${safeHubId(hubId)}.json`),
      MAX_CATALOG_CACHE_BYTES,
    )
  }

  async writeCatalogCache(hubId: string, value: unknown): Promise<void> {
    await atomicWriteJson(path.join(this.catalogsDir, `${safeHubId(hubId)}.json`), value)
  }

  async createInstallStaging(): Promise<string> {
  ...
  async commitRoleRoot(stagedRoleRoot: string, bundleDigest: string): Promise<string> {
  ...
  async removeStaging(directory: string): Promise<void> {
  ...
  async writeDeploymentRecord(record: RoleDeploymentRecord): Promise<void> {
    const validated = parseDeploymentRecord(record)
    await atomicWriteJson(
      path.join(this.deploymentsDir, `${validated.role.bundleDigest}.json`),
      validated,
    )
  }

  async listDeploymentRecords(): Promise<RoleDeploymentRecord[]> {
    ...
    for (const { filename, value } of records) {
      if (filename !== `${value.role.bundleDigest}.json`) {
        throw new Error(
          `rolehub-bridge: deployment receipt filename does not match ${value.role.bundleDigest}`,
        )
      }
    }
  ...
  async writeSessionBinding(binding: RoleSessionBinding): Promise<void> {
    ...
    await atomicWriteJson(
      path.join(this.sessionsDir, `${sessionFilename(validated.sessionId)}.json`),
      validated,
    )
  }

  async listSessionBindings(): Promise<RoleSessionBinding[]> {
    return readSessionBindingsStrict(this.sessionsDir)
  }
}

async function readSessionBindingsStrict(directory: string): Promise<RoleSessionBinding[]> {
  const files = await receiptFiles(directory)
  const values: RoleSessionBinding[] = []
...
async function readReceiptDirectory<T>(
  directory: string,
  parse: (value: unknown) => T,
): Promise<Array<{ filename: string; value: T }>> {
  const files = await receiptFiles(directory)
  const values: Array<{ filename: string; value: T }> = []
  for (const file of files) {
    values.push({
      filename: file.name,
...
async function receiptFiles(directory: string) {
...
async function atomicWriteJsonUnchecked(destination: string, value: unknown): Promise<void> {
...
async function claimStorageRoot(root: string): Promise<void> {
  const before = await safeLstat(root)
  if (before === undefined) {
    const created = await mkdir(root, { recursive: true, mode: PRIVATE_DIRECTORY_MODE })
    if (created !== undefined) await chmod(root, PRIVATE_DIRECTORY_MODE)
  }
  ...
  if (entries.length !== 0) {
    throw new Error(
      `rolehub-bridge: refusing non-empty unclaimed storage directory: ${root}`,
    )
  }
...
async function validateStorageMarker(marker: string): Promise<void> {
  const value = record(
    await readPrivateJson(marker, MAX_STORAGE_MARKER_BYTES),
    'storage ownership marker',
  )
  exactKeys(value, ['schemaVersion', 'owner'], 'storage ownership marker')
  if (
    value['schemaVersion'] !== STORAGE_MARKER_VALUE.schemaVersion ||
...
function assertNonDangerousStorageRoot(root: string): void {
  ...
  if (dshHome) dangerous.add(path.resolve(dshHome))
...
async function readPrivateJson(
  ...
  try {
    ...
    if (!stat.isFile()) throw new Error(`rolehub-bridge: receipt is not a regular file: ${file}`)
...
export function parseDeploymentRecord(value: unknown): RoleDeploymentRecord {
  ...
  if (root['schemaVersion'] !== DEPLOYMENT_SCHEMA_VERSION) invalid('deployment schemaVersion')
  ...
  for (const key of ['id', 'name', 'displayName', 'description', 'publisher', 'version'] as const) {
    string(role[key], `deployment role.${key}`)
  }
  stringArray(role['tags'], 'deployment role.tags')
  if (role['trust'] !== 'reference' && role['trust'] !== 'community') invalid('deployment role.trust')
  ...
  for (const [capability, tools] of Object.entries(bindings)) {
    if (!CAPABILITY_IDS.includes(capability as CapabilityId)) invalid(`binding capability ${capability}`)
    const names = stringArray(tools, `binding ${capability}`)
    if (names.some((name) => !TOOL_NAME.test(name))) invalid(`binding ${capability}`)
  }
  return value as RoleDeploymentRecord
}

export function parseSessionBinding(value: unknown): RoleSessionBinding {
  ...
  if (root['schemaVersion'] !== SESSION_BINDING_SCHEMA_VERSION) invalid('Session schemaVersion')
  for (const key of ['sessionId', 'parentSessionId', 'providerName'] as const) {
    boundedString(root[key], `Session ${key}`, 512)
  }
  digest(root['roleBundleDigest'], 'Session roleBundleDigest')
  if (root['roomId'] !== undefined) boundedString(root['roomId'], 'Session roomId', 512)
  if (root['roomMemberId'] !== undefined) boundedString(root['roomMemberId'], 'Session roomMemberId', 512)
  if (!['active', 'orphaned', 'revoked'].includes(String(root['state']))) invalid('Session state')
  isoDate(root['createdAt'], 'Session createdAt')
  return value as RoleSessionBinding
}

function parsePolicy(value: unknown): void {
  const policy = record(value, 'deployment policy')
  exactKeys(
    policy,
    ['apiVersion', 'kind', 'role', 'compatibility', 'grants', 'enforcement', 'policyDigest'],
    'deployment policy',
  )
  if (policy['apiVersion'] !== 'rolehub.dev/policy/v1alpha1') invalid('policy apiVersion')
  if (policy['kind'] !== 'EffectiveRolePolicy') invalid('policy kind')
  if (policy['compatibility'] !== 'dsharness') invalid('policy compatibility')
  ...
  if (grants.some((grant) => !CAPABILITY_IDS.includes(grant as CapabilityId))) invalid('policy grants')
  const enforcement = record(policy['enforcement'], 'policy enforcement')
  exactKeys(
    enforcement,
    ['filesystem', 'network', 'approvals', 'room', 'process', 'configuration'],
    'policy enforcement',
  )
...
function safeHubId(value: string): string {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(value)) invalid('Hub id')
  return value
}

function assertDigest(value: string): string {
  if (!SHA256.test(value)) invalid('bundle digest')
  return value
}
...
function exactKeys(
  ...
  if (unknown.length || missing.length) {
    throw new Error(
      `rolehub-bridge: invalid ${label} keys` +
...
function boundedString(value: unknown, label: string, maximum: number): asserts value is string {
  if (typeof value !== 'string' || !value || value.length > maximum || value.includes('\0')) invalid(label)
}

function stringArray(value: unknown, label: string): string[] {
  if (!Array.isArray(value) || !value.every((entry) => typeof entry === 'string')) invalid(label)
  if (new Set(value).size !== value.length) invalid(`${label} duplicates`)
...
function isoDate(value: unknown, label: string): void {
  ...
  if (!Number.isFinite(parsed.valueOf()) || parsed.toISOString() !== value) invalid(label)
}
