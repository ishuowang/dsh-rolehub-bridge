...
export type ParsedRoleHubCommand =
  ...
  | { action: 'start'; selector: string; label?: string; roomId?: string; prompt?: string }
  | { action: 'sessions' }
...
export function tokenizeRoleHubCommand(rawInput: string): string[] {
  const tokens: string[] = []
  let token = ''
  let started = false
  let quote: '"' | "'" | undefined
  ...
  for (let index = 0; index < rawInput.length; index += 1) {
    ...
    if (character === '\\') {
      ...
      if (escaped === undefined) throw new Error('rolehub: dangling escape at end of input')
      token += escaped
      started = true
      index += 1
      continue
  ...
  if (quote !== undefined) throw new Error(`rolehub: unterminated ${quote} quote`)
  finish()
  return tokens
}

export function parseRoleHubCommand(rawInput: string): ParsedRoleHubCommand {
  ...
  if (action === 'hubs' || action === 'list' || action === 'refresh' || action === 'sessions') {
    if (tokens.length !== 1) throw new Error(`rolehub: ${action} accepts no arguments\n${USAGE}`)
    return { action }
  }
  if (action === 'inspect') {
    if (tokens.length !== 2) throw new Error(`rolehub: inspect requires exactly one selector\n${USAGE}`)
...
function flags(
  ...
  for (let index = start; index < tokens.length; index += 2) {
    ...
    if (!allowed.includes(flag)) throw new Error(`rolehub: unknown flag "${flag}"\n${USAGE}`)
    if (result.has(flag)) throw new Error(`rolehub: duplicate flag "${flag}"`)
